...
| 模块 | 描述 |
|------|------|
| 权限信息定义 | 支持在资源注册时附带定义权限属性（名称、审批人、依赖） |
| 权限依赖级联 | 系统支持权限间的依赖关系，并在申请时自动处理 |

### 3.2 审批管理

| 模块 | 描述 |
|------|------|
| 审批流程配置 | 支持配置不同场景的动态审批流 |
| 资源注册审批 | 提供方注册 API/事件后，需经平台运营审批 |
| 权限申请审批 | 消费方申请权限后，需经资源提供方审批 |

### 3.3 API 资源管理

| 模块 | 描述 |
|------|------|
| API 多维分类 | 支持按应用类型和凭证类型构建多维分类视图 |
| API 注册 | 提供方提交 API 信息、分组、权限定义 |
| API 编辑 | 提供方修改已注册的 API 信息 |
| 分组管理（树形） | 支持提供方自定义树形分组结构 |

### 3.4 事件资源管理

| 模块 | 描述 |
|------|------|
| 事件多维分类 | 支持按应用类型、凭证类型构建事件分类 |
| 事件注册 | 提供方提交事件信息、通道类型、权限定义 |
| 事件编辑 | 提供方修改已注册的事件信息 |
...
| 模块 | 描述 |
|------|------|
| 目录浏览与检索 | 消费方可按分类树浏览已上架的 API/事件 |
| 权限多选申请 | 消费方可发起权限申请，支持多选 |
| 依赖自动级联 | 申请单自动处理权限依赖关系 |
| 订阅与配置管理 | 消费方管理已获批的权限订阅，配置事件通道 |
...
| **能力目录** | `/directory` | 按分类树浏览已上架的 API/事件，支持检索 | 消费方 |
| **资源管理** | `/resources` | 提供方注册/编辑 API/事件，管理分组树 | 提供方 |
