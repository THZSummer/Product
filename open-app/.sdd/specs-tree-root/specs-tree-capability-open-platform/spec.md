...
| G3 | 提供 API 管理能力 | 支持 API 的注册、编辑、关联分组、分类、树形分组管理；分类维度：应用类型（业务/个人）、凭证类型（应用类 A/B、开放应用凭证），多对多关系 |
| G4 | 提供事件管理能力 | 支持事件的注册、订阅管理、按 Topic 分发、分组、分类；事件消费区分通道类型（企业内部消息队列/WebSocket/WebHook），区分凭证类型（应用类 A/B）；支持按应用隔离消费 |
