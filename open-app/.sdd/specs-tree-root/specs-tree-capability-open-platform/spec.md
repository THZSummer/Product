...
> 💡 **核心设计**：权限对象不独立展示，而是作为 API/事件的属性附着在资源上。本模块定义权限的底层属性结构和级联规则。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-001 | 权限信息定义 | 支持在资源注册时附带定义权限属性（名称、审批人、依赖） | • 支持填写权限名称（显示名称）<br/>• 支持设置该权限的**审批人/审批组**<br/>• 支持选择已存在的权限作为**依赖权限** |
| FR-002 | 权限依赖级联 | 系统支持权限间的依赖关系，并在申请时自动处理 | • 支持一个权限依赖一个或多个其他权限<br/>• 消费方申请权限时，若选中含依赖的权限，**系统自动勾选依赖权限**<br/>• **被级联勾选的依赖权限不可取消** |

### 3.2 审批管理

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-003 | 审批流程配置 | 支持配置不同场景的动态审批流 | • 支持配置审批流适用场景（API 注册、事件注册、权限申请）<br/>• 支持条件路由（如高敏感度 API 需要额外审批节点） |
| FR-004 | 资源注册审批 | 提供方注册 API/事件后，需经平台运营审批 | • 注册提交后自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后资源变为「已上架」状态 |
| FR-005 | 权限申请审批 | 消费方申请权限后，需经资源提供方审批 | • 申请提交后，根据资源上定义的审批人自动生成审批单<br/>• 支持审批操作：**同意**（通过）、**驳回**（需填写原因）、**撤销**<br/>• 审批通过后自动激活订阅关系 |

### 3.3 API 资源管理

> 💡 **目录结构**：应用类型（业务/个人） -> 凭证类型（AKSK/应用类 A/B...） -> 分组（树形） -> 接口权限。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-006 | API 多维分类 | 支持按应用类型和凭证类型构建多维分类视图 | • 支持按**应用类型**（业务/个人）区分 API<br/>• 支持按**凭证类型**（AKSK/应用类 A/B...）进一步细分<br/>• 同一接口在不同凭证类型下视为**独立的资源记录** |
| FR-007 | API 注册 | 提供方提交 API 信息、分组、权限定义 | • 支持填写 API 基本信息（名称、路径、方法、文档链接）<br/>• 支持选择关联的**分组**<br/>• 支持填写**权限信息**（名称、审批人、依赖权限） |
| FR-008 | API 编辑 | 提供方修改已注册的 API 信息 | • 支持修改 API 基本信息、分组、权限定义<br/>• 已上架 API 修改后需**重新审批**（审批期间原配置继续生效）<br/>• 修改记录需留痕（操作人、时间） |
| FR-009 | 分组管理（树形） | 支持提供方自定义树形分组结构 | • 支持新增、编辑、删除分组节点<br/>• 支持拖拽或指定父节点构建层级关系<br/>• 删除分组时需处理关联的 API（如转移至其他分组） |

### 3.4 事件资源管理

> 💡 **目录结构**：类似 API，增加通道类型配置。

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-010 | 事件多维分类 | 支持按应用类型、凭证类型构建事件分类 | • 同 API 多维分类逻辑（应用类型 -> 凭证类型 -> 分组） |
| FR-011 | 事件注册 | 提供方提交事件信息、通道类型、权限定义 | • 支持填写事件基本信息（名称、Topic、文档链接）<br/>• 支持配置**通道类型**（WebHook、WebSocket、内部消息队列）<br/>• **WebSocket 通道仅限个人应用场景下配置**<br/>• 支持填写**权限信息**（名称、审批人、依赖权限） |
| FR-012 | 事件编辑 | 提供方修改已注册的事件信息 | • 支持修改事件基本信息、通道配置、权限定义<br/>• 已上架事件修改后需**重新审批** |

### 3.5 能力目录与订阅

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-013 | 目录浏览与检索 | 消费方可按分类树浏览已上架的 API/事件 | • 支持按**应用类型 -> 凭证类型 -> 分组**的层级展开浏览<br/>• 支持点击资源查看详细信息（文档、权限描述）及**跳转说明文档** |
| FR-014 | 权限多选申请 | 消费方可发起权限申请，支持多选 | • 支持在目录中勾选多个 API/事件权限加入申请单<br/>• 申请单需自动汇总所选权限及其对应的审批流 |
| FR-015 | 依赖自动级联 | 申请单自动处理权限依赖关系 | • 当选中含依赖的权限时，**自动关联加入依赖权限**（置灰不可取消）<br/>• 提交时若自动加入了新权限，需提示消费方 |
| FR-016 | 订阅与配置管理 | 消费方管理已获批的权限订阅，配置事件通道 | • 支持查看当前应用的已授权权限列表<br/>• 支持**取消订阅**权限<br/>• 支持配置事件的**消费通道**（如 WebHook URL、WebSocket Token 等） |
