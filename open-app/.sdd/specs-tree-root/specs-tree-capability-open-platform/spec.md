...
> ⚠️ **说明**：本 Feature 仅做**整体设计**，明确前后端功能边界与资源清单。子 Feature（如 `capability-open-platform-backend`、`capability-open-platform-frontend`）将包含各自的详细设计、Plan/Tasks 等。本 Feature **不包含**后续的 Plan/Tasks 过程。
...
> 📐 **本章写作标准**
> - **视角**：功能模块清单（回答"有哪些功能模块，前后端分工"）
> - **格式**：清单形式，列出模块、描述、归属（Backend/Frontend）
> - **粒度**：本 Feature 为整体设计，仅提功能清单，详细 FR 及验收标准见子 Feature
...
| 模块 | 描述 | 归属 |
|------|------|------|
| 权限信息定义 | 支持在资源注册时附带定义权限属性（名称、审批人、依赖） | Backend / Frontend |
| 权限依赖级联 | 系统支持权限间的依赖关系，并在申请时自动处理 | Backend / Frontend |

### 3.2 审批管理

| 模块 | 描述 | 归属 |
|------|------|------|
| 审批流程配置 | 支持配置不同场景的动态审批流 | Backend / Frontend |
| 资源注册审批 | 提供方注册 API/事件后，需经平台运营审批 | Backend / Frontend |
| 权限申请审批 | 消费方申请权限后，需经资源提供方审批 | Backend / Frontend |

### 3.3 API 资源管理

| 模块 | 描述 | 归属 |
|------|------|------|
| API 多维分类 | 支持按应用类型和凭证类型构建多维分类视图 | Backend / Frontend |
| API 注册 | 提供方提交 API 信息、分组、权限定义 | Backend / Frontend |
| API 编辑 | 提供方修改已注册的 API 信息 | Backend / Frontend |
| 分组管理（树形） | 支持提供方自定义树形分组结构 | Backend / Frontend |

### 3.4 事件资源管理

| 模块 | 描述 | 归属 |
|------|------|------|
| 事件多维分类 | 支持按应用类型、凭证类型构建事件分类 | Backend / Frontend |
| 事件注册 | 提供方提交事件信息、通道类型、权限定义 | Backend / Frontend |
| 事件编辑 | 提供方修改已注册的事件信息 | Backend / Frontend |

### 3.5 能力目录与订阅

| 模块 | 描述 | 归属 |
|------|------|------|
| 目录浏览与检索 | 消费方可按分类树浏览已上架的 API/事件 | Frontend |
| 权限多选申请 | 消费方可发起权限申请，支持多选 | Backend / Frontend |
| 依赖自动级联 | 申请单自动处理权限依赖关系 | Backend / Frontend |
| 订阅与配置管理 | 消费方管理已获批的权限订阅，配置事件通道 | Backend / Frontend |
