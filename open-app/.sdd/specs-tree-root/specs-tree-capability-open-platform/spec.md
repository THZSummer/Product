...
| # | 目标 | 衡量标准 |
|---|------|---------|
| G1 | 提供统一的能力开放基础设施 | 支持 API、事件、回调、连接器的全生命周期管理 |
| G2 | 建立统一的权限与审批底座 | 所有能力共享统一的权限模型和审批流程 |
| G3 | 实现能力提供方与消费方的自助服务 | 提供方在线注册/管理，消费方在线订阅/消费 |
| G4 | 支撑数据开放平台等上层应用 | 数据开放平台可复用权限管理、API/事件通道 |

### 1.5 Non-Goals

| # | 非目标 | 原因 |
|---|--------|------|
| NG1 | 实现能力消费网关（API 网关/流控） | 已有代码涉及较多企业内部逻辑，可能人工开发 |
| NG2 | 实现特有连接能力（IM 卡片、云盘、邮件） | 由业务模块建设，通过嵌入能力接入，本阶段仅搭建框架 |
| NG3 | 实现连接器管理 | Should Have，非 MVP 范围 |
| NG4 | 实现回调管理 | Should Have，非 MVP 范围 |
| NG5 | 实现能力目录/市场、开发者工具链、用量统计 | Should Have，非 MVP 范围 |
...
**验收标准**:
- [ ] 支持创建 API 分组（如 IM 分组、云盘分组）
- [ ] 支持 API 版本号（如 v1、v2）
- [ ] 支持版本间的文档差异展示
- [ ] 支持标记 API 为「已废弃」（Deprecated）
- [ ] 分组和版本信息在能力目录中可见
