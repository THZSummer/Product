...
| 需求编号 | 需求描述 | 验收标准 |
|---------|---------|---------|
| **MH-01** | 数据 Owner 能够注册数据对象权限 | 支持填写数据对象信息（名称、描述、敏感度），不关联通道 |
| **MH-02** | 通道 Owner 能够注册通道 | 支持填写通道描述，关联数据对象权限（多对多） |
| **MH-03** | 平台管理员能够审批 | 支持审批通过/驳回，记录审批意见 |
| **MH-04** | 三方平台能够创建应用 | 创建应用后获取 AK/SK |
| **MH-05** | 三方平台能够申请权限 | 申请 API 权限，后台自动生成数据对象权限申请单 |
| **MH-06** | 三方平台能够通过 API 消费数据 | 使用 AK/SK 调用 API 获取数据 |
| **MH-07** | 支持事件订阅形式 | 数据变更时推送给订阅方 |
| **MH-08** | 双重权限控制 | 应用需同时拥有通道权限 + 数据对象权限才能消费 |
| **MH-09** | 隔离审批流程 | 两条审批单（通道权限 + 数据对象权限），独立审批，各自动态生成审批链 |
| **MH-10** | 权限审计日志 | 以通道为入口记录调用日志，关联通道权限和数据对象权限信息 |
...
| **SH-02** | 数据对象敏感度分级 | 支持定义和管理数据对象敏感度，根据敏感度动态生成审批链 |
| **SH-03** | 我的权限清单 | 应用可查看已拥有的 API 列表，背后关联数据对象（平台内部） |
| **SH-04** | 用量统计 | 展示 API 被调用的次数、调用方等 |
| **SH-05** | 默认策略 | 通道权限默认附带基础数据对象权限，敏感数据对象权限需额外申请 |
...
| **SH-10** | 权限永久有效 | 权限永久有效，除非主动回收 |
| **SH-11** | AK/SK 重置无感 | 权限与应用 ID 绑定，AK/SK 更新不影响权限 |
| **SH-12** | 用户感知优化 | 消费方无感知数据对象，或可选提示依赖关系 |
...
从平台视角展示数据从注册到消费的完整流程，涉及数据 Owner、通道 Owner、平台管理员、消费方四角色。

```mermaid
sequenceDiagram
    participant DOwner as 数据 Owner
    participant COwner as 通道 Owner
    participant Platform as 数据开放平台
    participant Admin as 平台管理员
    participant Consumer as 三方平台应用
    
    rect rgb(200, 230, 255)
        note right of DOwner: 阶段 1：数据对象注册
        DOwner->>Platform: 注册数据对象权限
        note right of Platform: 填写数据对象信息<br/>（名称、描述、敏感度）
    end
    
    rect rgb(230, 255, 200)
        note right of COwner: 阶段 2：通道注册
        COwner->>Platform: 注册 API/事件通道
        COwner->>Platform: 关联数据对象权限（多对多）
        note right of Platform: 通道 Owner 自主关联<br/>无需数据 Owner 确认
    end
    
    rect rgb(255, 230, 200)
        note right of Admin: 阶段 3：审批上架
        Admin->>Admin: 数据对象审批（数据 Owner→平台管理员）
        Admin->>Admin: 通道审批（通道 Owner→平台管理员）
        Admin->>Platform: 审批通过
        Platform->>Platform: 数据对象 + 通道上架
    end
    
    rect rgb(230, 255, 200)
        note right of Consumer: 阶段 4：权限申请
        Consumer->>Platform: 创建应用获取 AK/SK
        Consumer->>Platform: 申请 API 权限
        note right of Platform: 系统提示依赖数据对象权限<br/>生成两条审批单
        Platform->>Admin: 通道权限审批单
        Platform->>Admin: 数据对象权限审批单
        Admin->>Admin: 并行审批（各自动态生成审批链）
        Admin->>Platform: 全部通过
        Platform->>Consumer: 授予消费权限（通道 + 数据对象）
    end
    
    rect rgb(255, 200, 230)
        note right of Consumer: 阶段 5：消费数据
        Consumer->>Platform: 调用 API 消费数据
        note right of Platform: 鉴权：检查通道权限 + 数据对象权限
        Platform->>Consumer: 返回数据
        Platform->>Platform: 记录审计日志（关联两种权限）
    end
    
    rect rgb(200, 255, 230)
        note right of DOwner: 阶段 6：运营监控
        Platform->>DOwner: 展示用量统计
        Platform->>DOwner: 价值反馈
    end
```
...
| 阶段 | 阶段三 | 阶段四 | 阶段五（当前） |
|------|--------|--------|---------------|
| 阶段 1 | 注册数据对象，关联通道 | 注册 API/事件通道，附带注册字段权限 | 注册数据对象权限，不关联通道 |
| 阶段 2 | 不适用 | 不适用 | 通道 Owner 注册通道，关联数据对象权限 |
| 阶段 3 | 动态生成审批链（基于数据对象敏感度） | API Owner+ 平台管理员审批 | 数据对象 + 通道独立审批 |
| 阶段 4 | 申请通道 + 数据对象权限，一条审批单 | 申请 API+ 字段权限，两条审批单并行审批 | 申请 API 权限，后台自动生成数据对象权限申请单 |
| 阶段 5 | 验证：通道权限 + 数据对象权限 | 验证：通道权限 + 字段权限 | 鉴权：检查通道权限 + 数据对象权限 |
| 审计 | 关联数据对象信息 | 关联字段权限信息 | 关联通道权限和数据对象权限信息 |
...
**核心设计原则**：永远申请的是权限，不是对象；消费权限 = 通道权限 + 数据对象权限

**平台定位**：数据开放网关，不触碰具体数据，管控权限元数据
...
graph TB
    ...
    subgraph 数据对象权限 DataObjPerm[数据对象权限]
        D1[员工信息对象权限]
        D2[群组对象权限]
        D3[文件对象权限]
    end
    ...
    subgraph 消费权限 AccessPerm[消费权限]
        P1[API 权限 + 员工信息对象权限]
        P2[Webhook 权限 + 群组对象权限]
        P3[MQ 权限 + 文件对象权限]
    end
    
    Application -->|申请 | ChannelPerm
    Application -->|申请 | DataObjPerm
    ChannelPerm & DataObjPerm -->|组合 | AccessPerm
    
    style Application fill:#e3f2fd
    style ChannelPerm fill:#fff3cd
    style DataObjPerm fill:#e1f5e1
...
| **数据对象权限** | 业务逻辑对象的访问权限（数据 Owner 注册时定义） |
| **组合规则** | 应用需同时拥有通道权限 + 数据对象权限才能消费数据 |
| **平台定位** | 网关角色，不触碰数据，管控权限元数据 |
| **用户感知** | 消费方无感知数据对象（或可选感知），数据 Owner 感知 |

**与阶段三/四的区别**：
| 维度 | 阶段三 | 阶段四 | 阶段五（当前） |
|------|--------|--------|---------------|
| 权限模型 | 通道 + 数据对象 | 通道 + 字段 | 通道权限 + 数据对象权限 |
| 数据概念 | 业务逻辑对象 | 抹除，面向逻辑字段 | 有对象概念，消费方无感知 |
| 平台定位 | 管理数据对象 | 平台不碰数据 | 网关角色，管控权限元数据 |
| 核心理念 | 对象思维 | 字段思维 | **权限思维** |
| 用户感知 | 双重感知 | 单一感知（API） | 消费方单一感知，Owner 双重感知 |

**示例**：
- 客服应用申请「获取群信息 API」权限时，系统提示该 API 依赖群组对象权限
- 后台自动生成两条申请单：API 通道权限申请单 + 群组对象权限申请单
- 消费方仅需要关注审批进度，不知道数据对象的存在（或可选提示）
- 鉴权时：平台网关检查 API 通道权限 ✅ + 群组对象权限 ✅，允许访问

#### 6.3.2 数据对象与通道关系

**数据 Owner 注册**：
- 注册逻辑数据对象权限（如：员工信息对象权限、群组对象权限）
- 不关联具体通道，只定义权限规则
- 不指定具体应用，降低管理成本

**通道 Owner 注册**：
- 注册 API/事件通道
- 关联已上架的数据对象权限（多对多，不做限制）
- 自主关联，无需数据 Owner 确认

**关系模式**：
- 多对多：一个数据对象可关联多个通道，一个通道可关联多个数据对象
- 平台不做限制，由通道 Owner 自主决定

**默认策略**：
- 通道权限默认附带基础数据对象权限
- 敏感数据对象权限需额外申请
- 申请 API 权限时，系统提示依赖关系，后台自动生成数据对象权限申请单

#### 6.3.3 权限审批流程

**核心原则**：两条审批单（隔离设计），并行审批，各自动态生成审批链
...
sequenceDiagram
    ...
    App->>Platform: 申请 API 权限
    note right of Platform: 系统检测依赖数据对象权限<br/>生成两条审批单
    
    par 并行审批
        Platform->>COwner: 通道权限审批单
        COwner->>Admin: 审批通过
        Admin->>Platform: 通道权限审批完成
        
        Platform->>DOwner: 数据对象权限审批单
        DOwner->>Security: 审批（根据敏感度）
        Security->>Admin: 审批通过
        Admin->>Platform: 数据对象权限审批完成
    end
    
    Platform->>Platform: 鉴权时需两个权限都有
...
| 维度 | 设计决策 |
|------|---------|
| **两条审批单** | 通道权限审批单 + 数据对象权限审批单，隔离设计更清晰 |
| **并行审批** | 两个审批单独立审批，提升效率 |
| **审批链生成** | 各自根据敏感度动态生成审批链 |
| **鉴权逻辑** | 检查通道权限 + 通道依赖的数据对象权限，两个权限都有才允许访问 |

**审批人视角**：
- 通道权限审批：通道 Owner 审批技术可行性，平台管理员审批规范性
- 数据对象权限审批：数据 Owner 审批业务合理性，平台管理员审批规范性，根据敏感度可能需要上级/CTO、安全合规审批
...
| 功能 | 设计决策 |
|------|---------|
| **我的权限清单** | 展示 API 列表，背后关联数据对象（平台内部） |
| **可申请的权限目录** | 展示 API/事件列表，提示依赖关系 |
| **申请进度可视化** | 非刚需，不需要展示详情 |
| **数据对象感知** | 消费方可选感知（提示依赖关系） |
...
| v1.18 | 2026-04-13 | 新增：阶段四字段权限挖掘 - 抹除数据对象概念、字段权限模型、并行审批 | AI Assistant |
| v1.19 | 2026-04-13 | 新增：阶段五权限思维挖掘 - 网关定位、权限思维、隔离审批、消费方无感知 | AI Assistant |
