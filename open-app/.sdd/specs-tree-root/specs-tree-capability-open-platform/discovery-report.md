...
flowchart TB
    ...
    subgraph Rebuild[重建模块/AI 开发]
        ...
        Note1["🔄 平滑过渡<br/>替代旧版模块<br/>共存/迁移旧数据"]
        R1 & R2 & R3 & R4 & R5 -.- Note1
    end
    ...
    subgraph Specific[特有连接能力/业务模块建设]
        ...
        S2[云盘业务能力]
        S3[会议业务能力]
        S4[IM 卡片能力<br/>现状：开放平台构建维护]
        S5[...]
    end
    
    %% 依赖与集成关系
    E1 -.->|依赖/对接 | R1 & R2 & R3 & R4
    E2 -.->|依赖/对接 | R2 & R4
    E3 -.->|依赖/对接 | R1 & R3
    
    %% 嵌入关系
    R1 & R3 -.->|嵌入/支撑 | S1 & S2 & S3
    
    style Existing fill:#fff3cd
    style Rebuild fill:#e1f5e1
    style New fill:#e8f5e9
    style Specific fill:#f3e5f5
    style Note1 fill:#ffffff,stroke:#f57c00,stroke-dasharray: 5 5
```

| 能力大类 | 能力子类 | 建设策略 | 开发方式 | 与现有系统关系 |
|---------|---------|---------|---------|-----------|
| **平台本身能力** | 应用管理、成员管理、AKSK | 沿用现有 | 复用 | 基础模块，被重建模块依赖 |
| | 操作日志、版本、文档等 | 沿用现有 | 复用 | 辅助模块，独立或被动记录 |
| **公共连接能力** | API 管理、API 权限管理 | 重新构建 | AI 开发 | **平滑过渡**：替代旧版 API 管理/权限，新模块需对接应用管理/成员管理 |
| | 事件与回调、事件权限 | 重新构建 | AI 开发 | **平滑过渡**：替代旧版事件管理，新模块需对接应用管理 |
| | 审批管理 | 重新构建 | AI 开发 | **平滑过渡**：替代旧版审批，新模块需对接应用/成员/权限管理 |
| | 连接器 | 完全新建 | AI 开发 | 无历史数据，独立建设 |
| **特有连接能力** | IM/云盘/会议等业务能力 | 业务模块建设 | 按需 | 嵌入开放平台，复用公共能力（API/事件） |
| | IM 卡片能力 | **沿用现有** | 复用 | **现状归属**：虽属特有连接能力，但目前由开放平台构建维护 |

> 💡 **XX 通讯平台开放平台现有能力清单**（11 项）：
> - **沿用现有（6 项）**：应用管理、成员管理、AKSK 凭证管理、操作日志、应用版本管理、开放文档管理
> - **重新构建（5 项）**：API 管理、API 权限管理、事件与回调管理、事件与回调权限管理、审批管理
> - **特有连接能力（现状沿用）**：IM 卡片能力（由开放平台构建维护，非 IM 业务模块）
>
> 🔄 **平滑过渡说明**：重新构建不是直接丢弃现有模块，而是 AI 开发新模块后，通过**共存运行、数据迁移、流量切换**逐步替代旧版功能；同时，新模块在设计上需**对接/依赖**现有基础模块（如应用管理、成员管理）。
