...
| union_id | string | 用户在 ISV 内的唯一标识，即在同一 ISV 下，该用户的不同应用的 union_id 相同 |
| email | string | 用户邮箱。该信息为管理员导入的用户联系方式，未经过用户本人实时验证，不建议开发者直接将其作为业务系统的登录凭证。如使用，务必自行认证。<br>**字段权限要求**：<br>获取用户邮箱信息 (contact:user.email:readonly) |
| enterprise_email | string | 企业邮箱，需要确保在管理后台已配置。<br>**字段权限要求**：<br>获取用户受雇信息 (contact:user.employee:readonly) |
