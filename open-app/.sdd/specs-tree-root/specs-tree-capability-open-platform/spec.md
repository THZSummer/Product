...
| # | 目标 | 衡量标准 |
|---|------|---------|
| G1 | 提供统一的权限管理底座 | 支持 Scope/RBAC 模型，可将权限分配给应用，支持权限鉴权 |
| G2 | 提供统一的审批管理底座 | 支持动态审批流配置，覆盖能力注册审批和能力订阅审批 |
| G3 | 提供 API 管理能力 | 支持 API 的注册、编辑、关联分组、分类、打标签、版本管理、状态管理；支持树形分组的新增/编辑/删除 |
| G4 | 提供事件管理能力 | 支持事件源注册、订阅管理、按 Topic 分发、状态管理 |
...
| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-01 | 作为 **IM 模块负责人**，我想要 **将 IM 的 API 注册到能力开放平台**，以便 **三方平台可以发现和使用 IM 能力** | P0 | 提供方可以填写 API 信息（名称、描述、路径、方法、参数），提交注册申请 |
| US-02 | 作为 **业务模块负责人**，我想要 **审批消费方的能力使用申请**，以便 **控制能力开放的风险** | P0 | 提供方可以查看待审批列表，通过/拒绝申请，审批记录留痕 |
| US-03 | 作为 **业务模块负责人**，我想要 **查看我的能力使用统计**，以便 **了解能力开放的价值** | P1 | 提供方可查看 API 调用量、消费方列表等统计信息 |

### 2.2 能力消费方（三方平台负责人）

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-04 | 作为 **三方平台负责人**，我想要 **浏览能力目录**，以便 **找到我需要的能力** | P0 | 消费方可以按分类（API/事件）浏览已上架的能力，查看 API 文档 |
| US-05 | 作为 **三方平台负责人**，我想要 **申请订阅能力**，以便 **获得调用权限** | P0 | 消费方可以发起订阅申请，选择应用和需要的能力，提交后等待审批 |
| US-06 | 作为 **三方平台负责人**，我想要 **通过 AK/SK 调用已授权的 API**，以便 **在业务中使用 XX 平台能力** | P0 | 消费方使用已授权的应用凭证调用 API，网关鉴权通过后返回数据 |

### 2.3 平台管理方（开放平台运营人员）

| ID | 用户故事 | 优先级 | 验收标准 |
|----|---------|--------|---------|
| US-07 | 作为 **平台运营人员**，我想要 **配置审批流程**，以便 **不同能力类型走不同的审批链** | P0 | 可以配置审批流程模板，根据能力类型/敏感度动态生成审批链 |
| US-08 | 作为 **平台运营人员**，我想要 **管理系统中的权限模型**，以便 **确保各能力权限正确分配** | P0 | 可以定义 Scope/Role，将权限分配给应用 |
...
**描述**: 系统提供统一的权限模型，支持 Scope/RBAC 两种模式。

**验收标准**:
- [ ] 支持定义 Scope 格式的权限标识（如 `api:im:message:read`）
- [ ] 支持定义角色（Role），角色可关联多个 Scope
- [ ] 支持将 Scope 或角色分配给应用（AppID）
- [ ] 权限标识遵循 `资源:操作:实例` 或 `模块:资源:操作` 格式
- [ ] 支持权限的 CRUD 操作

#### FR-002: 能力权限绑定

**描述**: 每个能力（API/事件）注册时需关联对应的权限标识。

**验收标准**:
- [ ] API 注册时可指定所需的 Scope
- [ ] 事件注册时可指定订阅所需的 Scope
- [ ] 能力上架后，其关联权限对消费方可见（用于申请）
- [ ] 支持一个能力关联多个 Scope

#### FR-003: 权限鉴权服务

**描述**: 提供权限鉴权服务，供 API 网关/事件订阅时调用。

**验收标准**:
- [ ] 提供鉴权 API 接口，输入 AppID + Scope，返回是否授权
- [ ] 支持缓存权限数据以提升鉴权性能
- [ ] 权限变更后，缓存需在合理时间内失效（可配置 TTL）
- [ ] 鉴权失败时返回明确的错误码

#### FR-004: 权限分配审批流

**描述**: 消费方申请能力权限时，触发权限分配流程。

**验收标准**:
- [ ] 消费方发起权限申请后，自动生成待办任务给能力提供方
- [ ] 权限分配记录与审批记录关联
- [ ] 权限分配成功后，消费方应用获得对应 Scope

### 3.2 审批管理

#### FR-005: 审批流程配置

**描述**: 支持动态配置审批流程，根据能力类型、敏感度等条件生成审批链。

**验收标准**:
- [ ] 支持配置审批节点（审批人、审批角色、审批方式）
- [ ] 支持条件路由（如：高敏感度能力需要平台运营人员审批）
- [ ] 支持串行/并行审批
- [ ] 支持审批流程模板的 CRUD 操作

#### FR-006: 能力注册审批

**描述**: 能力提供方注册能力后，需经过审批才能上架。

**验收标准**:
- [ ] 能力提交注册后自动生成审批单
- [ ] 审批单包含能力基本信息、提供方信息
- [ ] 审批通过：能力状态变为「已上架」
- [ ] 审批拒绝：能力状态变为「已拒绝」，提供方可查看拒绝原因
- [ ] 审批记录永久保存，支持审计查询

#### FR-007: 能力订阅审批

**描述**: 消费方申请订阅能力时，需经过能力提供方审批。

**验收标准**:
- [ ] 消费方发起订阅申请后自动生成审批单
- [ ] 审批单包含消费方应用信息、申请的能力列表、申请理由
- [ ] 审批通过：消费方应用获得对应权限
- [ ] 审批拒绝：消费方可查看拒绝原因
- [ ] 审批记录永久保存
...
**描述**: 审批相关方可查询审批状态，系统提供通知机制。

**验收标准**:
- [ ] 提供审批状态查询 API（待审批、已通过、已拒绝）
- [ ] 审批状态变更时通知相关方（可通过系统消息/邮件）
- [ ] 支持审批超时提醒（可配置超时时间）
...
**描述**: 能力提供方可以将 RESTful API 注册到平台。

**验收标准**:
- [ ] 支持填写 API 基本信息（名称、描述、提供方）
- [ ] 支持定义 API 路径（Path）和 HTTP 方法（GET/POST/PUT/DELETE 等）
- [ ] 支持定义请求参数（Header、Query、Path、Body）
- [ ] 支持定义响应格式（Schema）
- [ ] 支持设置文档信息（作为 API 属性，非独立功能）
- [ ] 注册后进入「待审批」状态

#### FR-010: API 编辑

**描述**: 能力提供方可以编辑已注册的 API 信息。

**验收标准**:
- [ ] 支持修改 API 基本信息（名称、描述、路径、方法等）
- [ ] 支持修改请求参数和响应格式
- [ ] 已上架的 API 编辑后需重新审批
- [ ] 编辑历史记录可追溯（操作人、变更时间、变更内容）
...
**验收标准**:
- [ ] 支持按**调用的应用类型**分类（如：企业内部应用、第三方应用、个人应用等）
- [ ] 支持按**凭证类型**分类（如：AKSK、OAuth Token 等）
- [ ] 分类信息在 API 注册/编辑时设置
- [ ] 分类信息在能力目录中可见，供消费方筛选
...
**验收标准**:
- [ ] 支持标签**「运行时需用户授权」**，标记该 API 调用时需要最终用户授权
- [ ] 支持自定义标签扩展
- [ ] 标签信息在 API 注册/编辑时设置
- [ ] 标签信息在能力目录中可见
...
**验收标准**:
- [ ] 支持状态流转：草稿 → 待审批 → 已上架 → 已下架 → 已废弃
- [ ] 状态变更需记录操作人和变更时间
- [ ] 已下架的 API 不再出现在能力目录中
- [ ] 已废弃的 API 在目录中标记为 Deprecated
...
**验收标准**:
- [ ] 支持**新增**分组，可指定父节点形成树形结构
- [ ] 支持**编辑**分组信息（名称、描述等）
- [ ] 支持**删除**分组（有子节点或关联 API 时提示处理）
- [ ] 分组支持层级展示，消费方可按树形结构浏览
...
**验收标准**:
- [ ] 支持填写事件基本信息（名称、描述、提供方、Topic）
- [ ] 支持定义事件 Schema（事件数据结构）
- [ ] 支持填写事件示例（Sample Payload）
- [ ] 支持填写事件触发条件说明
- [ ] 注册后进入「待审批」状态
...
**描述**: 平台支持按 Topic 进行事件广播分发。

**验收标准**:
- [ ] 事件发布时，根据 Topic 路由到所有已订阅的消费方
- [ ] 支持事件分发的可靠投递（至少一次）
- [ ] 支持事件重播（Replay）机制（消费方指定起始位点）
- [ ] 事件分发状态可查询（已发送、已确认、失败）
