...
| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-001 | 权限信息定义 | 支持在资源注册时附带定义权限属性 | • 支持填写权限名称（显示名称）<br/>• 支持设置该权限的**审批人/审批组**<br/>• **暂不支持权限依赖**（预留接口，未来数据对象等场景可能使用） |

### 3.2 审批管理

| FR | 名称 | 描述 | 验收标准 |
|----|------|------|---------|
| FR-003 | 审批流程配置 | 支持多层级审批流配置，支持从全局到特定资源的精细化控制 | • 支持**全局默认**审批流<br/>• 支持**场景特有**审批流（如 API、事件通用审批流）<br/>• 支持**资源特有**审批流（特定 API/事件独立配置，优先级最高）<br/>• 资源特有配置需支持为该资源及其**关联依赖**（如 API 关联的数据权限）分别指定审批流 |
