 ...
 */
export async function createSubFeature(featurePath, subFeatureId, name) {
 ...
 */
export async function generateSubFeatureIndex(featurePath) {
    const subFeaturesPath = path.join(featurePath, 'sub-features');
    // 如果是单模块模式，则返回空索引
 ...
 */
export async function scanSubFeatures(featurePath) {
    const mode = await detectFeatureMode(featurePath);
    if (mode === 'single') {
 ...
 * @returns 有效子目录名数组
 */
async function getSubFeatureDirectories(subFeaturesPath) {
    try {
        const items = await fs.readdir(subFeaturesPath);
