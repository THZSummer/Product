...
| **G20** | 企业内部消息数据消费通道 | 企业内部消息服务 | 消费者通过企业自研消息服务获取实时数据流 |
...
| 编号 | 功能 | 说明 |
|------|------|------|
| **G27** | 访问审计日志 | 记录所有 API 调用的详细日志（时间戳、用户、IP、请求路径、返回状态、耗时、访问数据量等），用于合规和安全分析 |
| **G28** | 审计日志查询 | 支持按应用/时间/状态/用户等条件查询审计日志，日志保留 ≥ 6 个月 |
...
| ID | 用户故事 | 验收标准 |
|----|----------|----------|
| **US-001** | 作为生产者（内部系统数据 Owner），我想要通过平台提供的注册接口注册数据集并配置数据源和更新频率，以便将数据开放给消费者使用 | 1. 平台提供数据集注册接口 2. 可配置数据源连接信息 3. 可设置数据更新频率 4. 注册后进入待审批状态 |
| **US-002** | 作为生产者（内部系统数据 Owner），我想要配置数据访问权限（对象/字段/范围/量/频率），以便控制消费者可访问的数据范围 | 1. 可选择开放的数据集 2. 可配置字段级权限 3. 可设置访问量和频率限制 4. 权限配置支持保存和修改 |

### 3.2 数据订阅管理

| ID | 用户故事 | 验收标准 |
|----|----------|----------|
| **US-003** | 作为消费者（外部系统），我想要订阅感兴趣的数据集，以便获取所需的数据 | 1. 可浏览已开放的数据集列表 2. 可查看数据集详情和权限要求 3. 可提交订阅申请 4. 订阅申请进入审批流程 |
| **US-004** | 作为消费者（外部系统），我想要配置数据推送目标（Webhook URL / MQ Topic），以便接收平台推送的数据 | 1. 可配置 Webhook 回调地址 2. 可配置消息队列订阅信息 3. 推送目标配置支持测试验证 4. 配置变更即时生效 |

### 3.3 审批管理

| ID | 用户故事 | 验收标准 |
|----|----------|----------|
| **US-005** | 作为企业管理员，我想要审批数据注册申请，以便确保开放数据符合企业规范和安全要求 | 1. 可查看数据注册申请详情 2. 可批准或拒绝申请 3. 审批记录被完整审计 4. 审批通过后数据集对消费者可见 |
| **US-006** | 作为生产者（内部系统数据 Owner），我想要审批消费者的数据订阅申请，以便确保数据被合规使用 | 1. 可查看订阅申请详情 2. 可批准或拒绝申请 3. 可填写审批意见 4. 审批结果通知申请人 |

### 3.4 数据通道

| ID | 用户故事 | 验收标准 |
|----|----------|----------|
| **US-007** | 作为生产者（内部系统），我想要通过 REST API / WebSocket / Kafka / RabbitMQ / 企业消息 / Webhook 推送数据到平台，以便将数据同步到开放平台 | 1. 支持多种数据推送方式 2. 数据推送支持认证和鉴权 3. 推送失败有重试机制 4. 推送状态可查询 |
| **US-008** | 作为消费者（外部系统），我想要通过 REST API / WebSocket / Kafka / RabbitMQ / 企业消息 / Webhook 获取数据，以便进行业务集成 | 1. 支持多种数据获取方式 2. 数据获取支持认证和鉴权 3. 返回数据符合权限配置 4. 响应时间满足性能要求 |
| **US-009** | 作为平台，我想要将生产者推送的数据路由到对应的消费者，以便实现数据分发 | 1. 根据订阅关系自动路由 2. 支持多种协议转换 3. 支持数据缓存和持久化 4. 支持数据格式转换和脱敏 |

### 3.5 数据权限管控

| ID | 用户故事 | 验收标准 |
|----|----------|----------|
| **US-010** | 作为平台，我想要根据权限配置控制消费者的数据访问（对象/字段/范围/量/频率），以便保障数据安全 | 1. 数据对象权限生效 2. 字段级权限自动过滤 3. 数据范围限制有效 4. 访问量和频率限制生效 5. 权限变更即时生效 |

### 3.6 数据使用审计

| ID | 用户故事 | 验收标准 |
|----|----------|----------|
| **US-011** | 作为企业管理员，我想要查询数据访问审计日志，以便进行合规分析和安全审计 | 1. 可查询所有 API 调用日志 2. 支持按应用/时间/状态等条件过滤 3. 日志保留 ≥ 6 个月 4. 日志数据不可篡改 |
...
- FR-001.3: 数据源类型必须支持：数据库（MySQL/Oracle/PostgreSQL）、API 接口、文件、消息队列
...
**描述**: 配置消费者可访问的数据对象/字段/范围/量/频率等权限

**需求**:
- FR-002.1: 系统必须支持数据对象权限配置（选择哪些数据集可访问）
- FR-002.2: 系统必须支持字段级权限配置（选择数据集中哪些字段可访问）
- FR-002.3: 系统必须支持数据范围权限配置（如部门范围、时间范围等）
- FR-002.4: 系统必须支持访问量限制配置（单次查询量、每日/月配额）
- FR-002.5: 系统必须支持访问频率限制配置（每分钟/小时/天调用次数）
- FR-002.6: 权限配置必须支持按消费者粒度分配
- FR-002.7: 权限变更必须即时生效
...
**描述**: 企业管理员审批数据注册申请和消费者访问申请

**需求**:
- FR-003.1: 系统必须提供审批任务列表，展示待审批的申请
- FR-003.2: 审批申请必须包含：申请人、申请时间、申请内容、权限要求等详细信息
- FR-003.3: 管理员必须能够批准或拒绝申请，并填写审批意见
- FR-003.4: 审批结果必须及时通知申请人
- FR-003.5: 必须记录所有审批操作日志
- FR-003.6: 必须支持多级审批流程配置
...
**描述**: 管理订阅关系、配置推送目标（Webhook URL、MQ Topic 等）

**需求**:
- FR-005.1: 系统必须支持配置 Webhook 推送地址
- FR-005.2: 系统必须支持配置消息队列订阅信息（Kafka Topic / RabbitMQ Queue）
- FR-005.3: 推送目标配置必须支持测试验证
- FR-005.4: 配置变更必须即时生效
- FR-005.5: 必须支持订阅关系的暂停、恢复、取消操作
- FR-005.6: 必须记录订阅推送日志
...
#### 4.3.1 数据生产者通道 (FR-006 ~ FR-011)

**FR-006 REST API 数据生产通道**:
- FR-006.1: 系统必须提供 REST API 接口供内部系统推送数据
- FR-006.2: API 推送必须支持认证（API Key / JWT）
- FR-006.3: 必须支持批量数据推送
- FR-006.4: 推送失败必须有重试机制

**FR-007 WebSocket 数据生产通道**:
- FR-007.1: 系统必须提供 WebSocket 服务端供内部系统连接
- FR-007.2: WebSocket 连接必须支持认证
- FR-007.3: 必须支持长连接心跳检测
- FR-007.4: 连接断开必须支持自动重连

**FR-008 Kafka 数据生产通道**:
- FR-008.1: 系统必须支持从 Kafka Topic 消费数据
- FR-008.2: 必须支持多 Topic 订阅
- FR-008.3: 必须支持消费组管理
- FR-008.4: 必须保证消息不丢失

**FR-009 RabbitMQ 数据生产通道**:
- FR-009.1: 系统必须支持从 RabbitMQ Exchange 消费数据
- FR-009.2: 必须支持多种 Exchange 类型（Direct/Topic/Fanout）
- FR-009.3: 必须支持消息确认机制
- FR-009.4: 必须支持死信队列处理

**FR-010 企业内部消息数据生产通道**:
- FR-010.1: 系统必须支持对接企业自研消息服务
- FR-010.2: 必须提供标准适配器接口
- FR-010.3: 必须支持消息格式转换

**FR-011 Webhook 数据生产通道**:
- FR-011.1: 系统必须支持配置内部系统的 Webhook URL
- FR-011.2: 必须支持按需主动拉取数据
- FR-011.3: 拉取失败必须有重试机制

#### 4.3.2 平台内部数据处理 (FR-012 ~ FR-015)
...
#### 4.3.3 数据消费者通道 (FR-016 ~ FR-021)

**FR-016 REST API 数据消费通道**:
- FR-016.1: 系统必须提供 REST API 接口供消费者查询数据
- FR-016.2: API 查询必须支持认证和鉴权
- FR-016.3: 必须支持分页、过滤、排序
- FR-016.4: 必须返回标准 JSON 响应格式

**FR-017 WebSocket 数据消费通道**:
- FR-017.1: 系统必须提供 WebSocket 服务端供消费者连接
- FR-017.2: 必须支持实时数据推送
- FR-017.3: 必须支持连接管理和心跳检测

**FR-018 Kafka 数据消费通道**:
- FR-018.1: 系统必须支持将数据发布到 Kafka Topic
- FR-018.2: 必须支持消费者订阅管理
- FR-018.3: 必须保证消息顺序和准确性

**FR-019 RabbitMQ 数据消费通道**:
- FR-019.1: 系统必须支持将数据发布到 RabbitMQ Queue
- FR-019.2: 必须支持多种消息类型
- FR-019.3: 必须支持消息确认机制

**FR-020 企业内部消息数据消费通道**:
- FR-020.1: 系统必须支持对接企业自研消息服务
- FR-020.2: 必须提供标准适配器接口

**FR-021 Webhook 数据消费通道**:
- FR-021.1: 系统必须支持向消费者的 Webhook URL 推送数据
- FR-021.2: 必须支持推送失败重试（最多 3 次）
- FR-021.3: 必须支持数字签名验证
...
| 测试 ID | 测试场景 | 负载条件 | 通过标准 |
|---------|----------|----------|----------|
| **PAT-001** | REST API 响应时间 | 1000 QPS 持续 5 分钟 | P95 ≤ 50ms |
| **PAT-002** | 系统吞吐量 | 逐步增加负载 | 稳定支撑 ≥ 5000 QPS |
| **PAT-003** | WebSocket 推送延迟 | 10000 并发连接 | P99 ≤ 100ms |
| **PAT-004** | 频率限制判定时间 | 高并发请求 | 判定耗时 ≤ 5ms |
| **PAT-005** | 内存占用 | 满载运行 | 峰值 ≤ 8GB |
...
| 版本 | 日期 | 作者 | 变更说明 |
|------|------|------|----------|
| 1.0.0 | 2026-03-30 | Summer | 初始版本 |
| 1.0.1 | 2026-03-30 | Summer | 规范审查修订：修正 1.1 背景占位符、简化 1.3 目标用户表格、修正第 2 章标题、统一验收标准格式 |
| 1.0.2 | 2026-03-30 | Summer | SDD Spec 审查更新：修复 WebSocket URL 拼写错误、补充用户故事验收标准、完善 FR-008 验收标准、增强错误处理表、扩展极端/并发场景、重构验收测试计划为可测试用例、添加 API 错误响应示例 |
| 1.0.3 | 2026-03-30 | Summer | 逐章审查修复：修正 Spring Boot 版本号 (3.4.15→3.4.x)、WebSocket URL 改为占位符格式 |
| 1.0.4 | 2026-03-30 | Summer | 逐章审查修复：修正 1.1 背景描述（open-app→XXX 系统）、更新元数据版本 (1.0.0→1.0.3) |
| 1.0.5 | 2026-03-30 | Summer | 逐章讨论确认：更新 1.1 背景定位、1.2 问题陈述、1.3 目标用户、1.4 技术栈（MyBatis/多消息队列/多认证方式）、新增 1.5 模块规划、第 2-11 章内容同步更新 |
| **1.0.6** | **2026-03-30** | **Summer** | **第 2 章重构：按数据注册管理/订阅管理/通道/权限管控/审计重组 G1-G28，第 3-12 章同步更新** |
