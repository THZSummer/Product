...
生产者 ──▶ US-001 注册数据集 ──▶ 填写元数据（名称/描述/数据源/更新频率）
                                    │
                                    ▼
                             US-002 配置生产方式
...
| 更新频率 | 使用场景 | 示例 |
|----------|----------|------|
| **实时** | 数据产生后立即推送到平台 | 订单创建、用户注册、支付完成 |
| **定时** | 按 cron 表达式定时同步 | 每天 2:00 同步昨日数据、每小时同步一次 |
| **手动** | 按需触发数据同步 | 初始化数据、补录数据、临时导出 |

#### 1.6.3 数据订阅流程（US-004 ~ US-006）
...
生产者 ──▶ US-009 配置授权要求 ──▶ 标记需要用户授权的数据集
                                    │
                                    ▼
消费者 ──▶ US-010 获取授权码 ──▶ 生成授权请求链接
                                    │
                                    ▼
普通用户 ──▶ US-011 管理授权 ──▶ 查看授权请求详情
                                    │
                              ┌─────┴─────┐
                              ▼           ▼
                         批准 ✓       拒绝 ✗
...
| 模式 | 技术栈 | 场景 |
|------|--------|------|
| **拉取 (Pull)** | REST API | 消费者按需查询、报表生成、数据初始化 |
| **推送 (Push)** | WebSocket/Kafka/RabbitMQ/Webhook | 实时数据同步、事件通知、消息订阅 |
