 ...
 * 
 * 输入：
 *   - src/templates/agents/*.md.hbs (8 个 agent 模板)
 *   - src/templates/config/opencode.json.hbs (1 个配置模板)
 * 
 * 输出：
 *   - dist/templates/agents/*.md (14 个 agent 定义)
...
function build() {
  ...
  console.log(`   Source: src/templates/agents/*.md.hbs (8 files)`);
  console.log(`   Output: dist/templates/agents/*.md (14 files)\n`);
  ...
  // 复制 sdd.md 和 sdd-help.md
  ['sdd', 'sdd-help'].forEach(name => {
  ...
  console.log('   ├── state/');
  console.log('   └── templates/agents/      (14 个 agent 定义)');
}
